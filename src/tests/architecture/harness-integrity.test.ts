 ...
 * All AI calls must go through src/lib/domain/ai/operations/ layer.
 *
...
describe('AI Harness Integrity', () => {
  test('no direct AIProvider imports outside allowed paths', () => {
    // Allowed paths: src/lib/domain/ai/providers/, src/lib/domain/ai/harness/, src/lib/domain/ai/singleton.ts
    ...
    const allowedPaths = [
      'src/lib/domain/ai/providers/',           // Provider implementations
  ...
  test('all AI operations have corresponding Zod schemas', () => {
    ...
    for (const name of operationNames) {
      const schemaPath = path.join(process.cwd(), 'src/lib/domain/ai/harness/schemas', `${name}.ts`)
  ...
  test('all AI operations have corresponding prompt templates', () => {
    ...
    for (const name of operationNames) {
      const promptPath = path.join(process.cwd(), 'src/lib/domain/ai/prompts', `${name}.ts`)
