...
import { enqueueHelpGeneration } from "@/lib/infra/queue";
...
describe('US-018: Progressive Help', () => {
  ...
  test('empty string does not count as new answer for unlock', async () => {
    ...
    const { submitCorrectionsSchema } = await import("@/lib/domain/validations/homework");
  ...
  test('locked level shows message explaining unlock requirement', async () => {
    ...
    const zhMessages = await import("../../i18n/messages/zh.json");
    const enMessages = await import("../../i18n/messages/en.json");
