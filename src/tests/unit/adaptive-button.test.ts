/**
 * Unit Tests: AdaptiveButton — tier-specific motion config dispatch
 */
...
describe("AdaptiveButton", () => {
  test("module exports AdaptiveButton with displayName", async () => {
    const mod = await import("@/components/adaptive/adaptive-button");
    expect(mod.AdaptiveButton).toBeDefined();
    expect(mod.AdaptiveButton.displayName).toBe("AdaptiveButton");
  });
