...
- [x] 34. 儿童友好 UI 适配（大字体大按钮主题）
- [x] 35. 响应式适配（PC + 手机）
- [x] 36. Docker 镜像构建 + 部署测试
- [x] 37. 端到端测试 + Bug 修复
...
- [x] 单张图片 AI 识别 ≤ 30 秒（实测 13.55s，5 题全部准确识别判分）
- [x] 页面首次加载 ≤ 3 秒（实测 0.2s）
- [x] HTTPS 传输 + 密码 bcrypt 哈希
- [x] 所有容器使用 `star-catcher-*` 前缀
