...
- [x] 从相册选择图片上传
- [x] 上传 PDF 识别
- [x] AI 正确识别试卷内容（文字+手写+公式）
- [x] AI 自动识别学科和内容类型
- [x] 用户可修正 AI 识别结果（内联编辑）
- [x] 第一轮检查显示对/错和得分，不显示答案
- [x] 学生改正后可重新提交检查
- [x] 多轮检查得分变化可见
- [x] 学生可对不会的题求助（渐进式揭示 3 级）
- [x] 家长可设置求助 Level 上限
- [x] 检查完成后错题自动录入错题库
- [x] 手动录入单题错题
- [x] AI Harness 管道完整运行（见 Harness 验收）
...
- [x] AI 重试失败后返回降级结果
- [x] 所有 AI 调用记录到 AICallLog
- [x] Rate Limit 通过 Redis 滑动窗口实现
- [x] 业务代码通过 Operations 层调用 AI
