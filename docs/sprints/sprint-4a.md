...
- [x] 39. Skill bundle 打包格式 + 脚手架 CLI + Schema Adapter
...
- [x] 40. SkillDefinition DB schema + 动态注册表
...
- [x] IPC 沙箱能安全执行 Skill 代码（隔离环境、超时终止、内存限制）
- [x] Skill bundle 打包 → 上传 → 注册 → 执行端到端流程通过
- [x] Skill 通过 IPC 调用 Harness 管道成功
- [x] Skill 通过 IPC 读写 Memory 层成功
- [x] 安全测试全部通过（文件系统/网络/env 访问被拦截）
- [x] SkillDefinition 表 CRUD 正常
- [x] SkillRegistry 正确过滤 ACTIVE Skill
- [x] Schema Adapter 正确将 Canonical JSON Schema 转为 OpenAI/Anthropic/Ollama 格式
