...
> **数据目录不用手动建**：compose 使用 Docker 命名卷（`star-catcher-pgdata`/`redisdata`/`miniodata`），Docker 自动创建并正确设置权限。
>
> **关于 NAS 提示的 PUID=1000 / PGID=10**：这是 linuxserver.io 社区镜像的约定，对我们用的官方 `pgvector/postgres`、`redis`、`minio` 镜像**不起作用**。命名卷方案直接绕过权限问题，无需配置 PUID/PGID。
...
# 创建目录结构（数据卷由 Docker 命名卷管理，无需预建子目录）
...
数据由 Docker 命名卷管理（`star-catcher-pgdata` / `redisdata` / `miniodata`），位于 Docker 存储区（通常 `/var/lib/docker/volumes/`）。无需手动 mkdir、chown、PUID/PGID。
...
| DB/Redis/MinIO 数据卷 | named volume（`star-catcher-*`） | named volume（`star-catcher-*`，Docker 自动管理权限） |
